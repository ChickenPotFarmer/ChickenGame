...
public class HarvestHub : MonoBehaviour
{
    [Header("Setup")]
    ...
    private AutoHarvestChick targetChick;
    ...
    [Header("Auto Chicks")]
    [SerializeField] private List<AutoHarvestChick> autoChicks; // add methods for altering chicks stats ie speed and shooting speed
    ...
    private void Start()
    {
        ...
        StartCoroutine(HubRoutine());
    }

    public bool TransferToHub(Transform _inventoryParent)
    {
        return inventoryController.InventoryToInventoryTransfer(_inventoryParent, objInventory.inventoryParent);
    }

    private IEnumerator HubRoutine()
    {
    ...
    public AutoHarvestChick GetAvailableChick()
    {
        AutoHarvestChick availChick = null;
    ...
    public WeedPlant GetAvailablePlant()
    {
        ...
        {
            if (plantsInRange[i].fullyGrown && plantsInRange[i].trimmed && !plantsInRange[i].harvested && !plantsInRange[i].targettedForHarvest)
            {
    ...
    public void UpdateAutoChickList()
    {
        ...
        {
            autoChicks.Add(autoChicksParent.GetChild(i).GetComponent<AutoHarvestChick>());
        }
    }
